...
function tpm() {
   mkdir -p "$TPMSTATE"
   mkdir -p "$HOME/.cache/libvirt/qemu/log/"
...
#THIS DOES WORK WITH VFIO AND WSL2
...
CPU="host,level=30" #THIS DOES NOT WORK WITH VFIO
#https://gitlab.com/qemu-project/qemu/-/issues/1031#note_2338766215
...
#### ISO
  -blockdev '{"driver":"file","filename":"/home/a/Downloads/virtio-win-0.1.271.iso","node-name":"libvirt-2-storage","auto-read-only":true,"discard":"unmap"}'
  -blockdev '{"node-name":"libvirt-2-format","read-only":true,"driver":"raw","file":"libvirt-2-storage"}'
  -device '{"driver":"ide-cd","bus":"ide.0","drive":"libvirt-2-format","id":"sata0-0-0"}'
  -blockdev '{"driver":"file","filename":"/home/a/Setup/iso/win11.iso","node-name":"libvirt-1-storage","auto-read-only":true,"discard":"unmap"}'
...
( cd $HOME/.local/lookinglass/module/
  ...
  bind_vfio
  echo -e "After qemu is started, run: \n   sudo chown \$USER:\$USER /dev/kvmfr0 ; looking-glass-client -f /dev/kvmfr0"
  sudo gdb qemu-system-x86_64 "${args[@]}"
  sudo rm -rf "$TPMSTATE/"*
  unbind_vfio
}
